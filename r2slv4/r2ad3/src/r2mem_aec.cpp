...
r2mem_aec::r2mem_aec(int iMicNum, r2_mic_info* pMicInfo_Aec, r2_mic_info* pMicInfo_AecRef, r2_mic_info*  m_pCpuInfo_Aec)
{
  ...
  m_iFrmLen_Last = R2_AUDIO_SAMPLE_RATE / 1000 * R2_AUDIO_AEC_FRAME_MS * 2 ;
  m_pData_Last = R2_SAFE_NEW_AR2(m_pData_Last,float,m_iMicNum,m_iFrmLen_Last);
  assert(m_iFrmLen_Last >= m_iFrmLen_Aec);
  
  if (m_iFrmLen_Last > m_iFrmLen_Aec) {
    m_pData_Tmp = R2_SAFE_NEW_AR1(m_pData_Tmp, float, m_iFrmLen_Last - m_iFrmLen_Aec) ;
  }else{
    m_pData_Tmp = NULL ;
  }
...
int r2mem_aec::process(float** pData_In, int iLen_In, float**& pData_Out, int& iLen_Out){
  ...
  while (cur < iLen_In) {
    ...
    if (m_iLen_In == m_iFrmLen_Aec) {
      ...
      for (int i = 0; i < m_iMicNum ; i ++) {
        if (m_iFrmLen_Last > m_iFrmLen_Aec) {
          memcpy(m_pData_Tmp, m_pData_Last[i] + m_iFrmLen_Aec, sizeof(float) * (m_iFrmLen_Last - m_iFrmLen_Aec));
          memcpy(m_pData_Last[i], m_pData_Tmp, sizeof(float) * (m_iFrmLen_Last - m_iFrmLen_Aec));
        }
        memcpy(m_pData_Last[i] + (m_iFrmLen_Last - m_iFrmLen_Aec) , m_pData_In[i], sizeof(float) * m_iLen_In);
      }
      m_iLen_In = 0 ;
    }
  }
