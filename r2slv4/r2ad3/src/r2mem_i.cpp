...
int r2mem_i::process(char* pData_In, int iLen_In, float**& pData_Out, int& iLen_Out){
  ...
  if (m_iInType == r2_in_int_24) {
    ...
    iLen_In = iLen_In / (sizeof(r2_int24) * m_pMicInfo_In->iMicNum);
    ...
    iLen_In = iLen_In / (sizeof(int) * m_pMicInfo_In->iMicNum) ;
    ...
    iLen_In = iLen_In / (sizeof(float) * m_pMicInfo_In->iMicNum) ;
