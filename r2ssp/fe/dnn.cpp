...
//#define FIXED_POINT
#define _ONLY_OUT_SVD

#ifdef USE_MKL  // use Intel MKL library
extern "C" {
#include "mkl_cblas.h"
#include "mkl.h"
}
...
*/
char* new_aligned( int size, int dim)
{
...
CDNN::CDNN(): total_layer_num(0), target_layer_num(0), model(NULL), status(false) 
{
...
*/
bool CDNN::load_model_org(const char* modelfile, int tgtid)
{
	...
	model = new Layer[target_layer_num-1];
	if (model==NULL)
	{
...
*/
bool CDNN::load_model_batch(const char* modelfile, int tgtid)
{
	FILE* fp = fopen(modelfile, "rb");
	if (fp==NULL)
	{
...
bool 
CDNN::get_batch_output_org(const float* cepbuffer, int num, int dim, \
...
bool 
CDNN::get_batch_output_batch(const float* cepbuffer, int num, int dim, \
									   int& outdim, float* outfea[], NetOutType type)
{
	/*
		cepbuffer	: [IN]	input feature buffer
	...
	*/
	...
	for(int i=1; i<target_layer_num-1; i++)
	{
		...
		//cblas_sgemm(CblasColMajor, CblasTrans, CblasNoTrans, model[i].curr_layer_num_stride, num, \
		//	model[i].prev_layer_num_stride, 1.0, \
		//	model[i].weights, model[i].prev_layer_num_stride, temp_activations[i-1], model[i].prev_layer_num_stride, \
		//	1.0, temp_activations[i], model[i].curr_layer_num_stride);
		...
		// intermediate layer
			...
			//// for SVD output layer.
#ifdef _ONLY_OUT_SVD
			if (i < target_layer_num - 3)
#else
			//// for SVD output+hidden layer.
			if (i % 2 == 0)
#endif
				for(int j=0; j<num; j++) {
					sigmoid(model[i].curr_layer_num, temp_activations[i]+j*model[i].curr_layer_num_stride);
				}
	}
...
void
CDNN::print_model_org(const char* outmod) const
{
	if (model == NULL)
	{
		printf("Error, empty model, load first!");
...
void
CDNN::print_model_batch(const char* outmod) const
{
	if (model == NULL)
	{
		printf("Error, empty model, load first!");
