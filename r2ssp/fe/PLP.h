...
#define LEFT_CONTEXT	5
#define RIGHT_CONTEXT	5
...
#define FUL_DIM			((VEC_DIM*3)*CONTEXT_SPAN)
#define ALN_FUL_DIM		((FUL_DIM+3)/4*4)
...
public:
	...
	//int		InitPar( char* szFEDir,float LoPass = -1,float HiPass = -1,bool bUsePow = true,bool bUsePitch = true, \
	//				 float LoWarpCut = -1.0f,float HiWarpCut = -1.0f,int nLifter = -1, \
	//				 int nFirstBuf = 150, int nIncBuf = 50, int nPitchDelay = 0, int nPitchHist = 1000, int nNormDelay = 0, int nNormHist = 1000, \
	//				 int nDelayOff = 2000, int nPreAvgWeight = 0, int nNullPitchDelay = 100);
	int		InitPar( char* szFEDir,float LoPass = -1,float HiPass = -1, \
					float WLoPass = -1,float WHiPass = -1, \
					bool bUsePow = true, bool bUseDither = true, \
...
//	int			m_nPitchHistory;
	...
	int			m_nDelayOff;
	bool			m_bUseDither;
//	int			m_nNullPitchDelay;
public:
	//³õÊ¼»¯£¬×¼±¸¿ªÊ¼Ò»ÌõÐÂµÄÓïÒô
	void Resume();
	//ÐÂÔö¼ÓÊý¾Ý
	int AddBuf( const float* pBuf,int iSamples );
	//¸øÒ»¸öÊÇ·ñ½áÊøµÄ±êÖ¾£¬²¢ÇëÇó×¼±¸Êä³öÊý¾Ý
//	int Norm_Diff( bool bEndflag );
	int Norm_Diff2( bool bEndflag );
	//·µ»ØÊý¾Ý
	void GetFE( float*& pFe,int*& pSNR,int& nFrame);

    float *GetEnergies() { return m_energies; }
...
private:
	...
	int			m_frmOffset;
//	int			m_ipitchSamples;
	//int			p_nframe;   //pitchµÄÖ¡´¦Àí,×¢Òâ´Ë´¦²»ÊÇÍ¨³£ÒâÒåÉÏµÄÖ¡
//	int			p_len;      //pitchÖ¡³¤£¬1000ms
...
//	float		m_tmpF0[MAX_SPEECH];
	...
	int			m_nbaseFrame;
	int			m_meanFeatureIdx; //×öCMVNÖ¡Êý±ê¼Ç
	int			m_varFeatureIdx; //×öCMVNÖ¡Êý±ê¼Ç
	int			m_normedFeatureIdx; //×öCMVNÖ¡Êý±ê¼Ç
//	int			m_PitchIdx;   //PitchÖ¡Êý±ê¼Ç
//	int			m_startPitchIdx;	//ÐèÒªÌápitchµÄÆðÊ¼Ö¡
	int			m_OutIdx;     //¿ÉÒÔ³ö´¦µÄÖ¡Êý
	float		m_LDAmat[VEC_DIM*CONTEXT_SPAN*FUL_DIM]; //LDA&MLLR±ä»»¾ØÕó
	int			m_LDAmatIdx; //×÷¾ØÕó±ä»»µÄÖ¡±ê¼Ç
	//bool		m_frontLDA;  //¿ªÍ·¼¸Ö¡¸´ÖÆ±ê¼Ç
...
#define PLP_MAX_ENERGY_FRAMES   100
    float m_energies[PLP_MAX_ENERGY_FRAMES];
};
