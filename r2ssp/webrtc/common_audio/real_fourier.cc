...
RealFourier::RealFourier(int fft_order) :
		order_(fft_order), omx_spec_(nullptr) {
	CHECK_GE(order_, 1);
	...
	ne10_fft_ = ne10_fft_alloc_r2c_float32(1<<order_);
#endif
}
...
RealFourier::fft_real_scoper RealFourier::AllocRealBuffer(int count) {
	return fft_real_scoper(
			static_cast<float*>(AlignedMalloc(sizeof(float) * count,
					kFftBufferAlignment)));
}

RealFourier::fft_cplx_scoper RealFourier::AllocCplxBuffer(int count) {
	return fft_cplx_scoper(
			static_cast<complex<float>*>(AlignedMalloc(
					sizeof(complex<float> ) * count, kFftBufferAlignment)));
}

void RealFourier::Forward(const float* src, complex<float>* dest) const {
#ifdef BF_USE_NE10_FFT
	ne10_fft_r2c_1d_float32((ne10_fft_cpx_float32_t*)dest, (ne10_float32_t*)src, ne10_fft_);
#else
	OMXResult r;
	r = omxSP_FFTFwd_RToCCS_F32(src, reinterpret_cast<OMX_F32*>(dest),
			omx_spec_);
	CHECK_EQ(r, OMX_Sts_NoErr);
#endif
}
...
#ifdef BF_USE_NE10_FFT
	ne10_fft_c2r_1d_float32(dest, (ne10_fft_cpx_float32_t*)src, ne10_fft_);
#else
	OMXResult r;
