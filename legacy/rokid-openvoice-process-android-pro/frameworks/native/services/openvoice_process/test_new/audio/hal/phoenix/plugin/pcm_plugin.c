...
#define PCM_PARAM_GET_MASK(p, n) &p->masks[n - SNDRV_PCM_HW_PARAM_FIRST_MASK];

enum { PCM_PLUG_HW_PARAM_SELECT_MIN,
...
static unsigned int param_list[] = {
    ...
    SNDRV_PCM_HW_PARAM_RATE,	    SNDRV_PCM_HW_PARAM_PERIOD_SIZE,
    SNDRV_PCM_HW_PARAM_PERIODS,
};

static const struct string_to_enum cmd_name_to_enum_table[] = {
    STRING_TO_ENUM(SNDRV_PCM_IOCTL_PREPARE),
    STRING_TO_ENUM(SNDRV_PCM_IOCTL_START),
    STRING_TO_ENUM(SNDRV_PCM_IOCTL_WRITEI_FRAMES),
    STRING_TO_ENUM(SNDRV_PCM_IOCTL_READI_FRAMES),
    STRING_TO_ENUM(SNDRV_PCM_IOCTL_HW_PARAMS),
};

const char *get_cmd_name(int cmd)
{
	int j;
	for (j = 0; j < ARRAY_SIZE(cmd_name_to_enum_table); j++) {
		if (cmd_name_to_enum_table[j].value == cmd) {
			return cmd_name_to_enum_table[j].name;
		}
	}
	return "NULL";
}
...
static void pcm_plug_set_interval(struct snd_pcm_hw_params *params, int p,
				  struct pcm_plugin_min_max *v, int is_integer)
{
...
static int pcm_plug_bytes_to_frames(unsigned int size, unsigned int frame_bits)
{
			       ...
			       struct snd_pcm_hw_params *params)
{
	...
	pcm_plug_set_interval(params, SNDRV_PCM_HW_PARAM_PERIOD_SIZE, &val, 1);
	...
	pcm_plug_set_interval(params, SNDRV_PCM_HW_PARAM_BUFFER_SIZE, &val, 1);
	return 0;
}
				 ...
				 struct snd_pcm_hw_params *c)
{
	...
	for (idx = 0; idx <= masks; idx++) {
		...
		/*
		 * set the changed mask if requested mask value is not the same
		 * as constrained mask value
		 */
		if (memcmp(req_mask, con_mask,
			   PCM_MASK_SIZE * sizeof(uint32_t)))
				    ...
				    struct snd_pcm_hw_params *c)
{
	...
	for (idx = 0; idx <= intervals; idx++) {
		...
		if (!(p->rmask &
		      (1 << (idx + SNDRV_PCM_HW_PARAM_FIRST_INTERVAL))))
			continue;
				     ...
				     struct snd_pcm_hw_params *c)
{
	...
	if (rc) {
		ALOGE("%s: masks refine failed %d", __func__, rc);
		return rc;
	}

	rc = pcm_plug_interval_refine(p, c);
	if (rc) {
		ALOGE("%s: interval refine failed %d", __func__, rc);
		return rc;
	}
			      ...
			      struct snd_pcm_hw_params *params)
{
	...
	if (rc) {
		ALOGE("%s: pcm_plug_get_params failed %d", __func__, rc);
		return -EINVAL;
	}
...
static int pcm_plug_interval_select(struct snd_pcm_hw_params *p,
				    unsigned int param, unsigned int select,
				    unsigned int val)
{
	struct snd_interval *i;
...
static void pcm_plug_hw_params_set(struct snd_pcm_hw_params *p)
{
	...
	pcm_plug_interval_select(p, SNDRV_PCM_HW_PARAM_FRAME_BITS, select,
				 val1);
	...
	pcm_plug_interval_select(p, SNDRV_PCM_HW_PARAM_BUFFER_SIZE, select,
				 val2);

	val2 = pcm_plug_frames_to_bytes(period_sz * periods,
					val1); // buffer_bytes;
	pcm_plug_interval_select(p, SNDRV_PCM_HW_PARAM_BUFFER_BYTES, select,
				 val2);
}
			    ...
			    struct snd_pcm_hw_params *params)
{
	...
	if (rc) {
		ALOGE("%s: __pcm_plug_hrefine failed %d", __func__, rc);
		return rc;
	}
			     ...
			     struct snd_pcm_sync_ptr *sync_ptr)
{
	...
	if (plugin->state >= PCM_PLUG_STATE_SETUP) {
		...
		if (ret == 0)
			sync_ptr->s.status.state =
			    convert_plugin_to_pcm_state(plugin->state);
	}
...
static int pcm_plug_ttstamp(struct pcm_plug_data *plug_data, int *tstamp)
{
	struct pcm_plugin *plugin = plug_data->plugin;
...
static int pcm_plug_open(unsigned int card, unsigned int device,
			 ...
			 struct snd_node *pcm_node)
{
	struct pcm_plug_data *plug_data;
	void *dl_hdl;
	int rc = 0;
	char *so_name = "/vendor/lib/libpcmplugin.so";
	...
	if (!dl_hdl) {
		ALOGE("%s: unable to open %s", __func__, so_name);
		goto err_dl_open;
	} else {
		ALOGI("%s: dlopen successful for %s, card: %d, device: %d",
		      __func__, so_name, card, device);
	}
	...
	if (!plug_data->ops) {
		ALOGE("%s: dlsym to open fn failed, err = '%s'", __func__,
		      dlerror());
		goto err_dlsym;
	}
	...
	if (rc) {
		ALOGE("%s: failed to open plugin", __func__);
		goto err_open;
	}
