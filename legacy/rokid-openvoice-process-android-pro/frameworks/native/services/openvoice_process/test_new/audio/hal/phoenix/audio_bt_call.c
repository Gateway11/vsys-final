...
struct pcm_config pcm_config_bt_in = {
    .channels = CHANNEL_STEREO, // ahub stereo. slot map mono
...
static struct pcm_config pcm_main_cap_codec_in = {
    .channels = CHANNEL_STEREO,
...
ecnr_param_t param = {
    ...
    .mic_out_cfg.ch = 2,  // Real ECNR lib may support arbitrary parameters(mono,stereo...)
...
static int btcall_create_resampler(struct alsa_audio_device *adev, int direction) {
    ...
    ch_cnt = BT_CHANNEL_CNT;//bt_data->config->channels;
...
static int open_codec_inpcm(struct alsa_audio_device *adev) {
    ...
    if (adev->platform->info.misc.ahub_en)
        ...
        bt_data->config = &pcm_main_cap_codec_in;
    if ((bt_data->card = sd_platform_get_card_id(adev->platform, AUDIO_NAME_PRIMARY)) < 0) {
        AALOGE("get card id failed");
        return -EINVAL;
    }
    bt_data->port = PORT_CODEC_IN;
...
#if 1
            if (adev->bt_data[BTCALL_BT_OUTPUT]->pcm) {
                //FP_WRITE(buf, size, mic_out_fp);
                ret = pcm_write(adev->bt_data[BTCALL_BT_OUTPUT]->pcm, buf, size);
                if (ret) {
