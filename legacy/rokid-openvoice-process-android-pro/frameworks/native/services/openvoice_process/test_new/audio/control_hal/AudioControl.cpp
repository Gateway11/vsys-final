...
AudioControl::AudioControl(): mFocusListener(nullptr), halBridge(nullptr) {}

Return<void> AudioControl::onDevicesToDuckChange(const hidl_array<DuckingInfo, 1>& duckingInfos) {
    if (halBridge)
        halBridge->onDevicesToDuckChange(duckingInfos);
    return Void();
}

Return<void> AudioControl::onDevicesToMuteChange(const hidl_array<MutingInfo, 1>& mutingInfos) {
    if (halBridge)
        halBridge->onDevicesToMuteChange(mutingInfos);
    return Void();
}

Return<int32_t> AudioControl::setAudioParamConfig(const AudioParamConfig& config) {
    if (halBridge)
        return halBridge->setAudioParamConfig(config);
    return -1;
}

Return<void> AudioControl::setAudioHalBridge(const sp<IAudioHalBridge>& halBridge) {
    this->halBridge = halBridge;
    return Void();
}

Return<void> AudioControl::requestAudioFocus(hidl_bitfield<AudioUsage> usage, int32_t zoneId,
...
Return<void> AudioControl::abandonAudioFocus(hidl_bitfield<AudioUsage> usage, int32_t zoneId) {
    if (mFocusListener)
