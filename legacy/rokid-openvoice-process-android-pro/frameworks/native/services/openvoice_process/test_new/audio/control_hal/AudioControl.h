...
class AudioControl : public IAudioControl {
  public:
    ...
    Return<void> setAudioHalBridge(const sp<IAudioHalBridge>& halBridge);
    Return<int32_t> setAudioParamConfig(const AudioParamConfig& config);
    Return<void> requestAudioFocus(hidl_bitfield<AudioUsage> usage, int32_t zoneId,
                                    hidl_bitfield<AudioFocusChange> focusGain);
    Return<void> abandonAudioFocus(hidl_bitfield<AudioUsage> usage, int32_t zoneId);
