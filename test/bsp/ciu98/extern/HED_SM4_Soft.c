...
{
    uint32_t r, dat_0, dat_1, dat_2, dat_3, mid;
    uint32_t s, key_0, key_1, key_2, key_3, rk[32];

    GET_UINT32(dat_0,  pstPara->iDataPtr,  0);
    GET_UINT32(dat_1,  pstPara->iDataPtr,  4);
    GET_UINT32(dat_2,  pstPara->iDataPtr,  8);
    GET_UINT32(dat_3,  pstPara->iDataPtr, 12);

    GET_UINT32(key_0,  pstPara->iKeyPtr,  0);
    GET_UINT32(key_1,  pstPara->iKeyPtr,  4);
    GET_UINT32(key_2,  pstPara->iKeyPtr,  8);
    GET_UINT32(key_3,  pstPara->iKeyPtr, 12);
    ...
    for (s = 0; s < 32; s += 4)
    {
        rk[s + 0] = key_0 ^= L2(BYTESUB(key_1 ^ key_2 ^ key_3 ^ CK[s + 0]));
        rk[s + 1] = key_1 ^= L2(BYTESUB(key_2 ^ key_3 ^ key_0 ^ CK[s + 1]));
        rk[s + 2] = key_2 ^= L2(BYTESUB(key_3 ^ key_0 ^ key_1 ^ CK[s + 2]));
        rk[s + 3] = key_3 ^= L2(BYTESUB(key_0 ^ key_1 ^ key_2 ^ CK[s + 3]));
    }
    ...
    for (r = 0; r < 32; r += 4)
    {
        dat_0 ^= L1(BYTESUB(dat_1 ^ dat_2 ^ dat_3 ^ rk[r + 0]));
        dat_1 ^= L1(BYTESUB(dat_2 ^ dat_3 ^ dat_0 ^ rk[r + 1]));
        dat_2 ^= L1(BYTESUB(dat_3 ^ dat_0 ^ dat_1 ^ rk[r + 2]));
        dat_3 ^= L1(BYTESUB(dat_0 ^ dat_1 ^ dat_2 ^ rk[r + 3]));
    }

    PUT_UINT32(dat_3, pstPara->oDataPtr,  0);
    PUT_UINT32(dat_2, pstPara->oDataPtr,  4);
    PUT_UINT32(dat_1, pstPara->oDataPtr,  8);
    PUT_UINT32(dat_0, pstPara->oDataPtr, 12);
...
{
    return SymmetricAlgModeProcess_Soft(SM4EcbFunc, pstPara, 16);
}
/************************************************************************************/
/**
  * @}
