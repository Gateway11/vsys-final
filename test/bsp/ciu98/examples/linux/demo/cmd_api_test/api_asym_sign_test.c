/******************************************************************************
 ...
 File name: 		api_asym_sign_test.c
 Author:			  liangww 
 Version:			  V1.0	
 Date:			    2021-06-25	
...
void helpmenu(void)
{
  	port_printf("\nHelp menu: api_asym_sign_test <option> ...<option>\n");
	  port_printf("option:- \n");
...
int main(int argc, char * argv[])
{
   ...
   if (argc < 2)
    {
      ...
      port_printf("--------------------------\n");
       ...
       while ((ch = getopt(argc, argv, "a:k:l:i:d:")) != -1)
       {
           switch (ch) 
        {
               case 'a':
  ...
  /****************************************api_asym_sign*****************************************/	
   ...
   if((a_flag == 1)&& (i_flag == 1)&& (l_flag == 1)&& (k_flag == 1)&& (d_flag == 1))
   {
        key.alg = alg_type;
	      key.id = kid;
        StringToByte(file_in_buf,in_buf,2*len);
      	...
      	ret = api_asym_sign (&key, &asym_param, in_buf, in_len, out_buf, &out_buf_len);//signature interface
        ...
        port_printf("\nthe output signed data length:%d\n",out_buf_len);
        port_printf("the output signed data :\n");
        for(i = 0;i<out_buf_len;i++)
      	{
      		port_printf("%02x",out_buf[i]);
      	}
        port_printf("\napi_asym_sign successfully!");
        port_printf("\n\n**************************************\n");
     }
     else
     {
