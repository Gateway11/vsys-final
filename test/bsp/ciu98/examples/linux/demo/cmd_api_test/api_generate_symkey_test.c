/******************************************************************************
 ...
 File name: 		api_generate_symkey_test.c
 Author:			  liangww 
 Version:			  V1.0	
 Date:			    2021-06-25	
...
void helpmenu(void)
{
  	port_printf("\nHelp menu: api_generate_symkey_test <option> ...<option>\n");
	  port_printf("option:- \n");
    port_printf("-a <length>      : alg type: ALG_AES128 : 0x60 \n");
    port_printf("                             ALG_DES128 : 0x00 \n"); 
    port_printf("                             ALG_SM4    : 0x40 \n");    
	  port_printf("-i <sym  kid> : sym_kid \n");
...
int main(int argc, char * argv[])
{
               ...
               case 'a':
                ...
                case 'i':
                       ...
                       id = trans_int(optarg); 
      				       if(id>0xFF)
      			        	{
                          port_printf("The sym key id must between 0x00 ~ 0xFF\n");
      						        exit(0);
      					      }
                       break;
  ...
  /****************************************api_generate_symkey*****************************************/	
   /*
	1.if the key is fixed, the write authentication is need obtained
	2.generate symmetric key
	*/
    //1.obtain the admin authentication
	 memcpy(trankey.val,key_buf,16);
	 trankey.val_len = transkey_len;
	 trankey.id = id;//the transport kid must be 0x02
	 pin.owner = ADMIN_PIN;
	 pin.pin_len = 0x10;
	 memcpy(pin.pin_value, pin_buf,pin.pin_len);
   ...
   if((a_flag == 1)&& (i_flag == 1))
   {
        ...
        symkey.alg = alg_type;
      	...
      	if ( ret != SE_SUCCESS)
      	{
      		LOGE("failed to api_generate_symkey\n");
      		return ret;
      	}
  
        port_printf("\n\n**************************************\n");
        port_printf("\napi_generate_symkey successfully\n");
        port_printf("\n\n**************************************\n");
     }
     else
     {
