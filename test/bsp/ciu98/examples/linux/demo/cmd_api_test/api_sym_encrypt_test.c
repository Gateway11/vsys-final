/******************************************************************************
 ...
 File name: 		api_sym_encrypt_test.c
 Author:			  liangww
 Version:			  V1.0
 Date:			    2021-06-25
...
void helpmenu(void)
{
  ...
  port_printf("-k <kid>               : The kid for the sym encrypt \n");
  port_printf("-l <input data length> : The input data length \n");
  port_printf("-i <input data file>       : The input data file \n");
  port_printf("-m <sym encrypt mode>  : sym_mode: SYM_MODE_CBC : 0x00 \n");
...
int main(int argc, char *argv[])
{
  ...
  if ((a_flag == 1) && (i_flag == 1) && (l_flag == 1) && (m_flag == 1) && (k_flag == 1) && (p_flag == 1))
  {
    ...
    ret = api_sym_encrypt(&key, &sym_param, in_buf, in_len, out_buf, &out_buf_len); // symmetric encryption
    if (ret != SE_SUCCESS)
    {
