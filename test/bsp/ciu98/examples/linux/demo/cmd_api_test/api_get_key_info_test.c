/******************************************************************************
 ...
 File name: 		api_get_key_info_test.c
 Author:			  liangww 
 Version:			  V1.0	
 Date:			    2021-06-25	
...
void helpmenu(void)
{
  	port_printf("\nHelp menu: api_get_key_info_test <option> ...<option>\n");
	  port_printf("option:- \n");
	  port_printf("-u <key usage>         : application : 0x01   \n");
    port_printf("                       : mangagement : 0x00   \n");             
           ...
           switch (ch) 
        {
                case 'u':
                       ...
                       usage = trans_int(optarg); 
      				        if((usage!=0x00)&&(usage!=0x01))
      			        	{
                          port_printf("The usage value must be 0x00 or 0x01\n");
      						        exit(0);
      					      }
                       break;
              case 'h': 
   ...
   //get key info
       if(usage==0x01)
        ...
        ret =api_get_key_info(if_app_key, out_buf,&out_buf_len);
      	if ( ret != SE_SUCCESS)
      	{
      		LOGE("failed to api_get_key_info!\n");
      		return ret;
      	}
        port_printf("\n\n**************************************\n");
        if(usage==0x01)
