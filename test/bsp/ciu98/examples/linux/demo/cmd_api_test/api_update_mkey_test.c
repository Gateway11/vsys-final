...
int main(int argc, char * argv[])
{
       ...
       while ((ch = getopt(argc, argv, "l:k:e:")) != -1)
       {
           switch (ch) 
        {
               case 'k':
                       ...
                       StringToByte(mkey_input, key_buf, 2*mkey_len);               
                       break;
                case 'l':
                       l_flag = 1;
  ...
  /****************************************api_update_mkey*****************************************/	
   /*
	1.obtain the device authentication
	2.update master key 
	*/
    //1.obtain the device authentication
     //get 16 bytes random
    api_get_random (0x10, random);
     //encrypt the random by SM4 to obtain the authentication data
   ...
   if((k_flag == 2)&& (l_flag == 1)&& (e_flag == 3))
   {
	  ...
	  memcpy(mkey_ori.val, mkey_old, mkey_len);
      mkey_ori.val_len = mkey_len;
