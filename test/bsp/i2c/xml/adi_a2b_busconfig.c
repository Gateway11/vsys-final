...
static void print_gen_sine_usage(const char *prog)
{
    printf(
        "Usage:\n"
        "  %s --gen-sine [sample_rate] [bit_depth] [channels] [duration_sec]\n"
        "Arguments:\n"
...
static int run_gen_sine_wav(int argc, char *argv[])
{
    char filename[128];
    ...
    if (argc > 2) sample_rate  = atoi(argv[2]);
    if (argc > 3) bit_depth    = atoi(argv[3]);
...
void processInterrupt(struct a2b_bus *bus, uint8_t addr) {
    ...
    adi_a2b_I2C_WriteRead(&g_handle, BUS_SELECT(bus, addr), 1, (uint8_t[]){A2B_REG_INTSRC}, 2, dataBuffer);
    if (dataBuffer[0]) {
