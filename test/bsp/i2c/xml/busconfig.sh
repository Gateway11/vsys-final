...
grep '<action' "${1:-adi_a2b_commandlist_spi.xml}" | while read -r action; do
    ...
    if [[ "${action#*Protocol=\"}" == I2C* ]]; then
        ...
        [[ "$instr" == "read" ]] && { \
            debug i2ctransfer -f -y "$dev_bus" "w$addr_width@$i2caddr$addr_bytes r$((len - addr_width))" || true; } || \
            debug i2ctransfer -f -y "$dev_bus" "w$len@$i2caddr$addr_bytes $(echo "$content" | sed "s/\([^ ]*\)/0x\1/g")"
    else
        ...
        case "$spiCmd" in  #2/7  MOSI  | 0x02 | NODE | ADDR | DATA[0] | ... | DATA[N-1] |
            00|02*|07|C*)  #0    MOSI  | 0x00 | ADDR | DATA[0] | ... | DATA[N-1] |          #C* MOSI  | 110b:LEN-1 | NODE | ADDR |
                debug spidev_test -D "$dev_bus" -s 1000000 -b 8 -v -p "$spi_cmd_bytes$addr_bytes$(echo ${content:+\\x${content// /\\x}})" ;;
            01|04)  #MOSI  | 0x01 | ADDR | LEN-1 | DUMMY   | ... | DUMMY     |
                debug spidev_test -D "$dev_bus" -s 1000000 -b 8 -v -p "$spi_cmd_bytes$addr_bytes$dummy" ;;
            05)     #MOSI  | 0x05 | DUMMY   | ... | DUMMY     |
                debug spidev_test -D "$dev_bus" -s 1000000 -b 8 -v -p "$spi_cmd_bytes$dummy" ;;
            *)
