...
static void *msg_handle_thread(void *) {
    ...
    ALOGI("thread(msg_handle_thread) start for factory AIDL");
    while(1) {
...
static void *msg_waiting_thread(void *s) {
    ...
    ALOGI("thread(msg_waiting_thread) start for factory AIDL");
    while(1) {
...
#define MMI_SOCKET "/dev/socket/mmi"
    ...
    ALOGI("create msg waiting thread(thread id=%lu) for factory aidl\n", pid_wait);
    ...
    ALOGI("create msg handle thread(thread id=%lu) for factory aidl\n", pid_handle);
